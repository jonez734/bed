...
| `deploy-venv` (dev/edit) | `bed/Makefile:deploy-venv` | n/a (deploy) | Default: builds wheels for `bbsengine6` + `bed` inline to `/tmp/bed-$$/` then installs into the **active** venv (the one that called `make deploy`). With `EDITABLE=1` (or `DEPLOY_EDITABLE=1` from `deploytool --editable`, or legacy `DEV=1`), runs `$(MAKE) -C src install` (`pip install -e .`) into the active venv — live edits visible without rebuild. Source-tree edits are picked up on next interpreter start. Behavior change vs. the previous `DEV=1` form: editable mode now installs into the active venv, not `/var/lib/bed/venv`. |
